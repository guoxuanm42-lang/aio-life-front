  ...
  <div class="food-record-page">
      ...
      <Spin :spinning="detailLoading">
        <template v-if="currentDetail">
          <div class="detail-title-row">
            <div>
              <h2>{{ currentDetail.record.dishName }}</h2>
              <Space wrap>
          ...
          <div class="review-grid">
            <div>
              <strong>口味描述</strong>
              <p>{{ currentDetail.record.tasteDescription || '—' }}</p>
            </div>
            <div>
              <strong>问题或不足</strong>
              <p>{{ currentDetail.record.problems || '—' }}</p>
            </div>
            <div>
              <strong>本次总结</strong>
              <p>{{ currentDetail.record.summary || '—' }}</p>
            </div>
            <div>
              <strong>下次改进</strong>
              <p>{{ currentDetail.record.nextImprove || '—' }}</p>
            </div>
            <div>
              <strong>是否值得复做</strong>
              <p>
                {{
                  currentDetail.record.worthRedo === undefined
                    ? '—'
                    : currentDetail.record.worthRedo
                      ? '是'
                      : '否'
                }}
              </p>
            ...
            <div>
              ...
              <p>{{ currentDetail.record.nextTrySuggestion || '—' }}</p>
            </div>
          </div>
    ...
    >
      <Form ref="formRef" :model="formState" :rules="rules" layout="vertical">
        ...
        <Divider orientation="left">图片管理</Divider>
